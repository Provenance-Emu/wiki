# Welcome to Provenance

**The premier multi-system emulator for iOS, tvOS, and visionOS.**

Play games from 38+ classic systems with beautiful library management, native Apple TV support, and museum-quality presentation. Available now on the **App Store**.

---

## 🚀 New Here? Start Here

### Get Provenance

**Recommended:** [Download from the App Store](https://apps.apple.com/us/app/provenance-app/id1596862805) (easiest, automatic updates)

**Alternative methods:** [Sideloading](installation-and-usage/installing-provenance/sideloading.md) or [Building from source](installation-and-usage/installing-provenance/building-from-source.md) (advanced users)

### First Time Setup

1. **Install** from App Store
2. **Add games** - See [Importing ROMs](installation-and-usage/roms/importing-roms.md)
3. **Start playing!**

Need help? Check the [FAQ](faqs.md) or visit our [Discord](https://discord.gg/provenance).

---

## ✨ What Makes Provenance Special

### 📺 Premier Apple TV Emulator
The **only** emulator built natively for tvOS. Play retro games on your big screen with proper controller support, TopShelf integration, and optimized performance.

### 🎨 Museum-Quality Library
Automatic artwork, manuals, and game information. Your collection looks beautiful.

### 🎮 38+ Systems Supported
From Atari 2600 to Nintendo 3DS, PlayStation to Sega Saturn. [See all systems →](info/supported-systems.md)

### 🔓 100% Open Source
Fully transparent, no tracking, community-driven. Free forever when sideloaded or built from source.

### ☁️ iCloud Sync
Seamlessly sync your library, saves, and settings across all your devices (Provenance Plus).

---

## 📖 Quick Navigation

### Getting Started
- [Installing Provenance](installation-and-usage/installing-provenance/) - App Store, sideloading, building
- [BIOS Requirements](installation-and-usage/bios-requirements.md) - Required files for certain systems
- [Importing ROMs](installation-and-usage/roms/importing-roms.md) - Add games to your library
- [FAQ](faqs.md) - Common questions answered

### Using Provenance
- [Supported Systems](info/supported-systems.md) - Full compatibility list
- [Controllers & Controls](info/controllers-and-controls/) - Supported controllers and setup
- [Game Saves](info/saves.md) - Save states and battery saves
- [Customizing ROMs](installation-and-usage/roms/customizing-roms.md) - Artwork, metadata, organization

### Advanced
- [Building from Source](installation-and-usage/installing-provenance/building-from-source.md) - For developers
- [Contributing](help/contribute.md) - Help improve Provenance
- [Troubleshooting](help/troubleshooting.md) - Fix common issues

---

## 🆕 What's New

### Recent Features
- 🎨 **Skins** - Customize on-screen controls per system
- ⚡ **Performance HUD** - Real-time FPS monitoring
- 🎮 **Enhanced Save States** - Improved save state selection UI
- 🔧 **Core Updates** - Latest emulation cores with better accuracy
- 🎬 **Shader Filters** - Visual enhancements for RetroArch/PPSSPP/Dolphin cores

[See full changelog →](https://github.com/Provenance-Emu/Provenance/releases)

---

## 🌟 Provenance Plus

**Optional subscription** ($3.99/month, $39.99/year, or $99.99 lifetime) to support development and unlock premium features:

- ☁️ **iCloud Sync** - Seamless library and save sync across devices
- 🧪 **Beta Access** - Test new features early via TestFlight
- 🎯 **Priority Support** - Get help faster
- ❤️ **Support Development** - Keep Provenance improving

**Note:** Provenance is **free forever** when sideloaded or built from source. Plus features support ongoing development.

---

## 💬 Community

- **Discord:** [discord.gg/provenance](https://discord.gg/provenance)
- **GitHub:** [github.com/Provenance-Emu/Provenance](https://github.com/Provenance-Emu/Provenance)
- **Issues:** [Report bugs](https://github.com/Provenance-Emu/Provenance/issues)
- **Reddit:** [r/Provenance](https://www.reddit.com/r/Provenance/)

---

## 📚 Documentation Structure

This wiki is organized into sections:

- **Installation & Usage** - Getting Provenance installed and adding games
- **Info** - System compatibility, controllers, technical details
- **Help** - FAQ, troubleshooting, contributing
- **Development** - Building from source, contributing code

Use the sidebar navigation to explore, or search for specific topics.

---

**Last updated:** February 2026  
**Provenance version:** Latest (App Store)  
**Wiki maintained by:** Provenance community
