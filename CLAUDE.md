...
- **`VOICE.md`** — voice rules for any public-facing prose (wiki copy is publicly visible — voice still applies).
...
- **`wiki/projects-index.md`** — registry of every active repo and how it relates to this one. Note: that index is in `personal-os/wiki/`, not here.
...
- The skins catalog website lives in a **separate repo** (`Provenance-Emu/skins`, cloned locally at `../skins`). The wiki links to it via `info/skins-guide.md` but does not contain its source files.
...
- **Spell Checker** (`.github/workflows/spellcheck.yml`) — Runs cspell on all markdown files. Custom dictionary in `.cspell/custom-dictionary.txt` (one term per line, alphabetical). To clear a spellcheck failure, add the term there.
- **Sync from Source** (`.github/workflows/sync-from-source.yml`) — Scheduled workflow that reads `Systems.plist` and Core plist files from the main Provenance repo and regenerates `info/system-reference.md`. Can be run locally: `python3 scripts/sync-from-source.py <path-to-provenance-repo> .`

**Do not manually edit `info/system-reference.md`** — it is fully auto-generated by the Sync from Source workflow and will be overwritten.

## PR Review Checklist

When reviewing documentation PRs (especially agent-generated ones), verify:
...
4. **Formatting** — Consistent Markdown style, no broken tables or unclosed GitBook tags
